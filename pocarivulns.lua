...
local function loadTabContent(tabName)
    if loadedTabs[tabName] then return loadedTabs[tabName] end
    
    local url = SCRIPT_URL .. Tabs[tabName]
    local success, contentFunc = pcall(function()
        return loadstring(game:HttpGet(url, true))()
    end)
    
    if success and type(contentFunc) == "function" then
        ...
        contentFunc(contentFrame, player, Activate)
        
        loadedTabs[tabName] = contentFrame
        return contentFrame
    else
        warn("Failed to load tab:", tabName)
        return nil
    end
end
...
for name, tab in pairs(tabs) do
    tab.button.MouseButton1Click:Connect(function()
        if name == "Main" or Activate then
            for _, content in pairs(loadedTabs) do
                content.Visible = false
            end
            
            local content = loadedTabs[name] or loadTabContent(name)
            if content then
