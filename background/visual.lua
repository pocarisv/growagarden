...
createTab("Infinite Seed Pack/Egg Loader", 5, true)
...
local function createInfiniteLoaderContent()
    ...
    title.Position = UDim2.new(0, 0, 0, 10)
    title.BackgroundTransparency = 1
    title.Text = "∞ Infinite Seed Pack/Egg Loader"
    title.Font = Enum.Font.GothamBold
    title.TextSize = 16
    title.TextColor3 = Color3.fromRGB(180, 185, 230)
    title.TextXAlignment = Enum.TextXAlignment.Center
    ...
    infiniteBtn.Text = "🔃 ACTIVATE INFINITE LOAD"
    ...
    local function applyVisualQuantity(tool, quantity)
        ...
        if not originalQuantities[tool] then
            originalQuantities[tool] = qtyInfo.quantity
        end
        
        tool.Name = createVisualName(qtyInfo, quantity)
    end
    
    local function restoreOriginalQuantity(tool)
        if not tool or not tool.Parent then return end
        if not originalQuantities[tool] then return end
        
        local qtyInfo = extractQuantityInfo(tool.Name)
        if not qtyInfo then return end
        
        tool.Name = createVisualName(qtyInfo, originalQuantities[tool])
    end
    ...
    local function startInfiniteLoad()
        ...
        lastTool = tool
        statusLabel.Text = "🔄 Initializing infinite load..."
        
        for i = 5, 1, -1 do
            if not infiniteModeActive then return end
            statusLabel.Text = "⏳ Initializing in " .. i .. "s..."
            updateProgress((5 - i) * 20)
            wait(1)
        end
        
        local startQty = originalQuantities[tool] or extractQuantityInfo(tool.Name).quantity
        statusLabel.Text = "🚀 Loading infinite amount..."
        ...
        if infiniteModeActive then
            statusLabel.Text = "✅ Infinite load complete!"
            applyVisualQuantity(tool, TARGET_QUANTITY)
        end
        
        wait(2)
        if infiniteModeActive then
            statusLabel.Text = "∞ Infinite amount active"
            updateProgress(100)
        else
            updateProgress(0)
        end
    ...
    infiniteBtn.MouseButton1Click:Connect(function()
        ...
        if infiniteModeActive then
            infiniteBtn.Text = "⏹ STOP INFINITE LOAD"
            ...
            for tool, _ in pairs(originalQuantities) do
                if tool and tool.Parent then
                    restoreOriginalQuantity(tool)
                end
            end
            originalQuantities = {}
    ...
    local function onCharacterAdded(character)
        character.ChildAdded:Connect(function(child)
            if child:IsA("Tool") then
                ...
                if infiniteModeActive and extractQuantityInfo(child.Name) then
                    applyVisualQuantity(child, TARGET_QUANTITY)
                end
            end
        end)
    end
    ...
    local function onBackpackChildAdded(child)
        if child:IsA("Tool") and infiniteModeActive and extractQuantityInfo(child.Name) then
            applyVisualQuantity(child, TARGET_QUANTITY)
        end
    end
    ...
    if backpack then
        ...
        for _, tool in ipairs(backpack:GetChildren()) do
            if tool:IsA("Tool") and infiniteModeActive and extractQuantityInfo(tool.Name) then
                applyVisualQuantity(tool, TARGET_QUANTITY)
            end
        end
    end
...
tabContents["Infinite Seed Pack/Egg Loader"] = createInfiniteLoaderContent()
